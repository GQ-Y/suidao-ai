...
import { X, Search, Maximize, Minimize } from 'lucide-react';
...
export default function Dashboard({ onClose }: DashboardProps) {
  ...
  const topControls = (
     <div className="flex items-center justify-end gap-4 text-[11px] font-mono font-bold tracking-widest uppercase">
        ...
        <button 
          ...
          className="px-4 py-1.5 border rounded-sm transition-all shadow-[0_0_10px_rgba(0,0,0,0.5)] flex items-center gap-2 bg-[#051020] border-cyan-800 text-cyan-600 hover:border-cyan-500 hover:text-cyan-300"
        >
  ...
  return (
    <div className={`fixed inset-0 z-40 flex items-center justify-center font-sans text-cyan-50 ${isFullscreen ? '' : 'pt-8 pb-4 px-4'}`}>
      <div className={`w-full ${isFullscreen ? 'h-full max-w-none' : 'max-w-[1500px] h-full'} bg-[#020610]/95 tech-grid cyber-panel flex flex-col font-mono relative backdrop-blur-xl transition-all duration-300`}>
        ...
        {!isFullscreen ? (
          ...
          <div className="flex-1 flex overflow-hidden relative">
            <div className="absolute inset-0 z-0 bg-[#02050c]">
              <TunnelVisualizer onDeviceClick={(id, hasWarning) => hasWarning ? setShowHandling(true) : setShowMedia('live')} isMaintenance={isMaintenance} isDirtAlarm={isDirtAlarm} fullScreen={true} />
            </div>
            <div className="absolute left-0 top-0 bottom-0 z-10 w-[320px] bg-transparent border-r border-cyan-900/30 flex flex-col shadow-[10px_0_30px_rgba(0,0,0,0.5)] opacity-50 transition-all duration-300 hover:opacity-100 hover:bg-[#02050c]/80 hover:backdrop-blur-md">
               <SidebarStats onViewMedia={() => setShowMedia('live')} />
            </div>
